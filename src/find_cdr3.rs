...
impl OutputRecord {
    pub fn fmt_seq(&self) -> String {
        match self {
            OutputRecord::Neither => String::from("\t\t\t"),
            OutputRecord::Forward(name, cdr3, full_seq) => format!(
                "{}\t{}\t{}",
                name,
                std::str::from_utf8(cdr3).unwrap(),
                crate::reference::translate(full_seq),
            ),
            OutputRecord::Reverse(name, cdr3, full_seq) => format!(
                "{}\t{}\t{}",
                name,
                std::str::from_utf8(cdr3).unwrap(),
                crate::reference::translate(full_seq),
...
fn find_cdr3(
    ...
    let v_matches = reference_seqs.iter().map(
        |reference::RefV {
            ...
            let mut matches = owned_myers.find_all_lazy(seq, edit_dist);
